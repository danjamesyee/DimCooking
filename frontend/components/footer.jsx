import React from "react";

const Footer = () => (
  <div className="footer">
    <div className="flexer">
      <div className="foot-label">
        <h1>ABOUT US</h1>
        Dim Cooking is heavily inspired by the New York Times Cooking website.
        It was created by Daniel Yee, a software engineer based in San
        Francisco, CA.
      </div>
      <div className="learn">
        <h1 className="foo">LEARN MORE</h1>
        <div>
          <a href="https://github.com/danjamesyee">
            <div>Github</div>
          </a>
        </div>
        <div>
          <a href="https://www.linkedin.com/in/daniel-yee-6886441b0/">
            <div>LinkedIn</div>
          </a>
        </div>
        <div>
          <a href="https://cooking.nytimes.com/">Original Site</a>
        </div>

        <div>
          <a href="https://thewoksoflife.com/">Original Recipes</a>
        </div>
      </div>
    </div>
  </div>
);

export default Footer;
