...
export const fetchSavedRecipes = () => (dispatch) =>
  SavedRecipeUtil.fetchSavedRecipes().then((savedRecipes) =>
    dispatch(receiveSavedRecipes(savedRecipes))
  );

export const fetchSavedRecipe = (savedRecipeId) => (dispatch) =>
  SavedRecipeUtil.fetchSavedRecipe(savedRecipeId).then((savedRecipe) =>
    dispatch(receiveSavedRecipe(savedRecipe))
  );

export const createSavedRecipe = (savedRecipe) => (dispatch) =>
  SavedRecipeUtil.createSavedRecipe(savedRecipe).then((savedRecipe) =>
    dispatch(receiveSavedRecipe(savedRecipe))
  );

export const deleteSavedRecipe = (savedRecipeId) => (dispatch) =>
  SavedRecipeUtil.deleteSavedRecipe(savedRecipeId).then(() =>
